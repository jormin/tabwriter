...
func check(
	t *testing.T, testname string, minwidth, tabwidth, padding int, padchar byte, flags uint, src, expected string,
) {
	var b buffer
	b.init(1000)
...
func BenchmarkTable(b *testing.B) {
	for _, w := range [...]int{1, 10, 100} {
		...
		for _, h := range [...]int{10, 1000, 100000} {
			b.Run(
				fmt.Sprintf("%dx%d", w, h), func(b *testing.B) {
					b.Run(
...
func BenchmarkPyramid(b *testing.B) {
	for _, x := range [...]int{10, 100, 1000} {
		...
		b.Run(
			fmt.Sprintf("%d", x), func(b *testing.B) {
				b.ReportAllocs()
				for i := 0; i < b.N; i++ {
					w := NewWriter(io.Discard, 4, 4, 1, ' ', 0) // no particular reason for these settings
...
func BenchmarkRagged(b *testing.B) {
	...
	for _, h := range [...]int{10, 100, 1000} {
		b.Run(
			fmt.Sprintf("%d", h), func(b *testing.B) {
				b.ReportAllocs()
				for i := 0; i < b.N; i++ {
					w := NewWriter(io.Discard, 4, 4, 1, ' ', 0) // no particular reason for these settings
